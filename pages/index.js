import Head from 'next/head' import { useState, useEffect } from 'react' import { motion } from 'framer-motion' import { FaTwitter, FaGithub, FaSun, FaMoon } from 'react-icons/fa' import Typewriter from 'typewriter-effect'

export default function Home() { const [darkMode, setDarkMode] = useState(true)

useEffect(() => { if (darkMode) { document.documentElement.classList.add('dark') } else { document.documentElement.classList.remove('dark') } }, [darkMode])

return ( <> <Head> <title>Daycreed</title> </Head> <main className="min-h-screen bg-white dark:bg-black text-black dark:text-white flex flex-col items-center justify-center p-8 relative overflow-hidden">

{/* Background kapal bajak laut */}
    <motion.img
      src="/pirate-ship.png"
      alt="Pirate Ship"
      className="absolute w-[700px] opacity-10 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none"
